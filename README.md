...
1. В файле problems/knapsack_problem/brute_force_solver.py необходимо реализовать метод *get_knapsack* класса *BruteForceSolver* для поиска набора вещей рюкзака максимальной стоимости с применением полного перебора, использовав при этом реализованные методы *get_weight*, *get_cost* в родительском классе. Для генерации всех вариантов компоновки рюкзака использовать числа от 1 до $2^n - 1$ представленные в бинарном виде, то есть в виде строки из нулей и единиц длиной $n$, где $n$ - количество предметов для комплектации рюкзака.
