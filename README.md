# Задание 7. Варианты 1-4
# Оптимальное расписание. Конвейерная задача
## Задачи  
1.  Конвейерная задача - в файле `schedules/conveyor_schedule.py` реализовать два приватных метода класса `ConveyorSchedule`.
...
2. Отдельные задачи для вариантов:
    - Вариант 1. Разработать диаграмму классов пакета schedules с использованием библиотеки (mermaid-js)[https://github.com/mermaid-js/mermaid#class-diagram-docs---live-editor]. Добавить новые методы для класса `AbstractSchedule`, для вывода времени простоя для каждого исполнителя и общего времени простоя для всего расписания так, чтобы методы работали для расписания, построенного любым алгоритмом (для различных задач из теории расписаний). Добавить тесты для реализованных методов.
    - Вариант 2. Реализовать для класса `AbstractSchedule` вывод полученного расписания в формате диаграммы Ганта библиотеки (mermaid-js)[https://github.com/mermaid-js/mermaid#gantt-chart-docs---live-editor] так, чтобы метод работал для расписания, построенного любым алгоритмом (для различных задач из теории расписаний).
    - Вариант 3. Добавить метод для класса `ConveyorSchedule`, для изменения состава задач и перерасчета расписания. Добавить тесты для реализованного метода.
    - Вариант 4. Добавить отдельные методы для класса `ConveyorSchedule`, для добавления/удаления задачи и перерасчета расписания. Добавить тесты для реализованных методов.
...
- В пакете представлены следующие классы
    ...
    * `ConveyorSchedule`: Составляет оптимальное расписание для указанного списка задач. Для каждого исполнителя расписание представлено набором экземпляров класса `ScheduleItem`.
